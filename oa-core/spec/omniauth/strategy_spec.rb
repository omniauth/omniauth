...
describe OmniAuth::Strategy do
  ...
  describe '#call' do
    ...
    context 'custom callback suffix' do
      before do
        @options = {:callback_suffix => 'wowzers'}
      end

      it 'should use a custom suffix for callback' do
        lambda{ strategy.call(make_env('/auth/test/wowzers')) }.should raise_error("Callback Phase")
      end

      context 'callback_url' do
        it 'uses a custom callback suffix' do
          strategy.should_receive(:full_host).and_return('http://example.com')

          lambda{ strategy.call(make_env('/auth/test')) }.should raise_error("Request Phase")

          strategy.callback_url.should == 'http://example.com/auth/test/wowzers'
        end
        ...
        it 'preserves the query parameters' do
          ...
          strategy.callback_url.should == 'http://example.com/auth/test/wowzers?id=5'
        end
      end
    end
